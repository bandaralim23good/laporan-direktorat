  ...
  try {
    ...
    for (const row of values) {
      ...
      const data = {
        ...
        pentolQam: parsePercentage(row[1]),
        pentolQhs: parsePercentage(row[2]),

        lumpiaQam: parsePercentage(row[3]),
        lumpiaQhs: parsePercentage(row[4]),

        apQam: parsePercentage(row[5]),
        apQhs: parsePercentage(row[6]),

        ukQam: parsePercentage(row[7]),
        ukQhs: parsePercentage(row[8]),
      };

      const result = await prisma.qamProductReport.upsert({
        where: {
          year_month: {
            year,
            month,
    ...
    console.error("QAM Product Sync Error:", error);
